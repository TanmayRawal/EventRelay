...
class TokenBucketRateLimiter {
     ...
     */
    async consume(key, capacity, refillRatePerSec) {
    ...
    `;
        try {
            const result = await client_1.redis.eval(luaScript, 1, redisKey, capacity.toString(), refillRatePerSec.toString(), now.toString());
            return result === 1;
        }
        catch (err) {
